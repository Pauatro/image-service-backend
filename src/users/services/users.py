...
from users.services.exceptions import UserNotFoundException, UserAlreadyExistsException, IncorrectUsernameOrPasswordException
from users.services.authentication import verify_password

def get_user_by_username(username: str, session: Session = get_db_session(), logger: Logger = get_logger()) -> User:
    try: 
...
def create_user(user: CreateUser, session: Session = get_db_session(), logger: Logger = get_logger()):
    try:
        existing_user = get_user_by_username(user.username, session, logger)
        if existing_user:
            logger.error(f'Failed to create user with username {user.username} because it already exists')
