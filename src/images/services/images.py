...
def get_patient_name(path: str, logger: Logger = get_logger()) -> str:
    try:
        ...
        logger.log(
            f"Image in {path} was accessed succesfully and the patient name was retrieved"
        )
        return patient_name
...
def get_image_pixel_array(path: str, logger: Logger = get_logger()):
    try:
        ...
        logger.log(
            f"Image in {path} was accessed succesfully and the image pixel data was retrieved"
        )
        return pixel_array
    except Exception as error:
        logger.error(f"Failed to access image in {path} or its patient name: {error}")
...
def get_image_dir_by_position(
    position_getter: Callable[[int], int], logger: Logger = get_logger()
):
    image_set_folders = os.listdir(settings.image_db_directory)

    if (not image_set_folders) | len(image_set_folders) == 0:
        logger.error(
            f"Failed to access the image db directory {settings.image_db_directory} or it was empty"
        )
        raise ImageNotFoundException()
